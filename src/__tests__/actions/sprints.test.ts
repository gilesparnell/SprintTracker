...
describe("Sprint Actions", () => {
  ...
  it("should get a sprint by id", () => {
    const created = createSprint(db, {
      name: "Sprint 1",
      startDate: "2026-03-27",
      endDate: "2026-04-10",
    });

    const sprint = getSprintById(db, created.sprint!.id);
    expect(sprint).toBeDefined();
    expect(sprint!.name).toBe("Sprint 1");
  });

  it("should list all sprints sorted by start date", () => {
    createSprint(db, {
      name: "Sprint B",
  ...
  it("should update a sprint", () => {
    ...
    const result = updateSprint(db, created.sprint!.id, {
    ...
    expect(result.success).toBe(true);
    const updated = getSprintById(db, created.sprint!.id);
    expect(updated!.name).toBe("Sprint 1 - Updated");
    expect(updated!.goal).toBe("New goal");
    expect(updated!.status).toBe("active");
  });

  it("should delete a sprint and cascade delete its tasks", () => {
    const created = createSprint(db, {
      name: "Sprint 1",
      startDate: "2026-03-27",
      endDate: "2026-04-10",
    ...
    const result = deleteSprint(db, created.sprint!.id);
    expect(result.success).toBe(true);
